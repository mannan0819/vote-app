...
export function MOption({ option, votesData, votes, totalVotes, onChange, checkedId, ended }: MOptionProps): ReactElement {
    return <label
    ...
    >
        ...
        <p className={"ml-2 pl-1 text-2xl font-bold " +
            ((!!votesData && votes?.some(v => v.optionId === option.id)) || ended ?
                "bg-yellow-50 contents text-blue-800" : "text-white")}
            style={(votesData || ended) ? {
